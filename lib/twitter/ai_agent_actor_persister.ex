defmodule Twitter.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%Twitter.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(Twitter.Accounts.User, id, authorize?: false) do
